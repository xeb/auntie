...
use browser::{BrowserServer, BrowserServerConfig};
use std::path::PathBuf;
use std::process;

use db::BrowserDb;
use output::{format_error, format_result};
...
fn run(cli: Cli) -> Result<()> {
    ...
    match &cli.command {
        ...
        Command::Stop => {
            ...
            let result = browser::BrowserResult {
                success: true,
                message: if killed {
...
fn output(result: &browser::BrowserResult, json_mode: bool) {
